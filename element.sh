...
then
  ...
  then
    ...
    do
      ATOMIC_NUMBER_CLEAN=$(echo $ATOMIC_NUMBER | sed 's/ //')
      echo -e "The element with atomic number $ATOMIC_NUMBER_CLEAN is $NAME ($SYMBOL). It's a $TYPE, with a mass of $MASS amu. $NAME has a melting point of $MELTING_POINT celsius and a boiling point of $BOILING_POINT celsius."
    done
  fi
else
